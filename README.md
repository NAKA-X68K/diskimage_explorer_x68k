...
2. `Create New Image` から空の `HDF` / `HDS` / `XDF` を作成、または `Mount` で既存イメージを開く
...
- `Create New Image` ボタンから `New HDS` / `New HDF` / `New XDF` を選択できます。
- 保存先ディレクトリとファイル名を指定して `Create` を押すと空イメージを作成します。
- `HDF` / `HDS` では容量を MB 単位で自由入力できます。
- 容量欄は数値のみ入力できます。
- ファイル名に拡張子がない場合は自動で補完します。
- `HDF/HDS` は 1MB 以上の値を指定できます。
- 新規 `HDF` / `HDS` は X68000 の SASI / SCSI パーティション形式で作成されます。
- `XDF` は固定で `2HD 1232KB` です。
